...
**→ MR2 Phase-5 产品化 ⏳ Block 5.1 + 5.1.5 ✅ 通过。MP4 逐帧读取 + RVM 推理 + 视频合成输出验证通过（C++ 与 Python baseline 一致）。Block 5.2~5.4 待续。优化实验 s18: 5.8-s18-1~5 ✅（Frontend 纯分离架构），5.8-s18-6~8 🔜（MPP 硬件解码接入）。← 你在这里**
...
| **Phase-3** | C++ 集成 — Block 3.0~3.6 接口泛化 + RecurrentStateManager + Pipeline RVM 路径 | ✅ 2026-04-13 板端5帧通过，~27ms@256×256 FP16 |
| **4.1** | INT8 校准数据集选择：300 张 P3M train | ✅ 2026-04-13 |
| **4.2** | INT8 RKNN 转换：5.5MB 纯 INT8（FP16 9.0MB → 39% 压缩）。混合量化因 SDK bug 阻塞 | ✅ 2026-04-13 |
| **4.3** | 板端 benchmark（INT8 vs FP16），目标 <25ms @256×256 | ✅ 2026-04-14 **16ms**（提速 1.67×） |
| **4.4** | 质量评估：INT8 ≈ FP16，不需要混合精度 | ✅ 2026-04-14 |
| **4.5** | 长视频漂移：100 帧递归零 NaN/零溢出 | ✅ 2026-04-14 |
| **4.6** | 混合精度（跳过：纯 INT8 质量足够） | ⏭️ 跳过 |
| **5.1** | InputSource 抽象 + Mp4InputSource + 视频逐帧循环 + Frontend 拆分 | ✅ 2026-04-15 175帧通过，15~17ms/帧 |
| **5.1.5** | 视频合成输出（VideoWriter + alpha compositing）+ SIGINT 优雅退出 | ✅ 2026-04-15 C++ 与 Python baseline 一致 |
| **5.2** | 双 buffer 流水线（读帧/推理解耦） | ⏳ |
| **5.3** | 30fps 目标优化 | ⏳ |
| **5.4** | 稳定性测试 | ⏳ |
...
| 2026-04-15 | InputSource 抽象 + unique_ptr 所有权 | 为 Phase-6 V4L2/IPC 预留扩展，禁用 shared_ptr/raw pointer |
| 2026-04-15 | 视频模式不挂 GuidedFilter | GF 设计用于单图发丝增强，逐帧跑影响 30fps 目标 |
| 2026-04-15 | FFmpeg 精准关闭 X11/VAAPI/VDPAU | RK3588S headless 环境不需要，避免 40+ 系统包依赖 |
| 2026-04-15 | VideoWriter alpha compositing 蓝色默认背景 | 绿色背景无法验证抠图效果，蓝色可视化区分度高 |
| 2026-04-15 | SIGINT 全局原子变量跨翻译单元 | static → extern，pipeline.cpp 可检查 Ctrl+C 信号优雅退出 |
| 2026-05-16 | Frontend 纯分离架构（5.8-s18-1~5） | _IInputSource + _IFrameDecoder + _Preprocessor，MPP 路径和 OpenCV 路径独立 |
| 2026-05-16 | 命名空间 arcforge → helmsman | 删除 embedded 层，统一为 helmsman::utils 等单层命名空间 |
| 2026-05-19 | MPP 硬件解码接入（5.8-s18-6~8） | --mpp 标志，FFmpeg demux + MPP decode + RGA NV12→BGR |
