...
**→ Round 2 RVM Phase-1 已启动（Gate-0 GO ✅ 2026-04-09）← 你在这里**
...
> Gate-0 GO 2026-04-09，Phase-1 启动。
> PKB 入口：`model/round2-rvm/plan-MR2-roadmap.md`

| Block | 描述 | 状态 |
|---|---|---|
| **1.0** | submodule 添加 + 环境搭建 + MattingNetwork 可导入验证 | ✅ 2026-04-10 |
| **1.1** | ONNX 导出精调：去除 refiner、固化 downsample_ratio | ⏳ |
| **1.2** | PC 端 Python 推理验证（逐帧 + 递归状态 shape） | ⏳ |
| **1.3** | ONNX 图简化（onnxsim） | ⏳ |
| **1.4** | ArcFoundry FP16 配置创建 | ⏳ |
