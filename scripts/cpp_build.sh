...
# ==============================================================================
# [REFACTORED] Level 8: Cpp action execution (Top layer)
# ==============================================================================

# ------------------------------------------------------------------------------
# [REFACTORED] func_8_5: Pure Conan Preparation
# This function NO LONGER runs CMake. It only prepares dependencies.
# ------------------------------------------------------------------------------
func_8_5_cpp_run_native_conan_install() {
    ...
    func_1_1_log "   Target Dir: ${CPP_BUILD_WITH_PLATFORM_DIR}"
    func_1_1_log "   Build Type: ${CPP_BUILD_TYPE}"

    # Map build type for Conan
    local conan_build_type="Release"
    if [[ "${CPP_BUILD_TYPE}" == "debug" ]]; then
        conan_build_type="Debug"
    fi

    # Ensure build directory exists (It matches the Preset's binaryDir)
    mkdir -p "${CPP_BUILD_WITH_PLATFORM_DIR}"

    # Execute Conan Install
    # We use --output-folder to force Conan to put files exactly where CMake Preset expects the build to be.
...
# ------------------------------------------------------------------------------
# [REFACTORED & FIXED] func_8_4: Unified Dispatcher
# Fixes the bug where "cb" command deletes Conan files after they are generated.
# ------------------------------------------------------------------------------
func_8_4_cpp_dispatch() {
    ...
    # If the action implies cleaning, do it BEFORE generating any dependencies.
    # ==========================================================================
    if [ "$CPP_ACTION" == "clean" ] || [ "$CPP_ACTION" == "cb" ]; then
        func_7_1_cpp_core_clean_target

        # If the user only wanted to clean, stop here.
        if [ "$CPP_ACTION" == "clean" ]; then
            return 0
        fi
    fi

    # ==========================================================================
    # PHASE 2: DEPENDENCY GENERATION (Native Only)
    # Now that the directory is clean (if requested), we generate files.
    # ==========================================================================
    ...
    # ==========================================================================
    # PHASE 3: MAIN EXECUTION (Configure / Build / Install / Test)
    # Note: We removed 'clean' from the case statements because it's handled in Phase 1.
    # ==========================================================================
    case "$CPP_ACTION" in
        "list")
            cmake --list-presets
        ...
        "build")
            # Incremental Build
            func_7_2_cpp_core_configure "${extra_cmake_args}"
            func_7_3_cpp_core_build
            ;;

        "cb")
            # Fresh Build (Clean is already done in Phase 1)
            # Now we just Configure -> Build -> Install
            func_7_2_cpp_core_configure "${extra_cmake_args}"
            func_7_3_cpp_core_build
            func_7_4_cpp_core_install
            ;;
        ...
        "test")
            func_7_2_cpp_core_configure "${extra_cmake_args} -DBUILD_TEST=TRUE"
            func_7_3_cpp_core_build
            func_7_5_cpp_core_test
            ;;

        *)
            func_1_2_err "Unknown command '$CPP_ACTION'"
            ;;
    esac
}
